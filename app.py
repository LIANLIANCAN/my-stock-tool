    ...
    try:
        ...
        <div class="ai-insight">
            ...
            <ul style="line-height: 1.6;">
                <li><b>財務動能：</b>本月營收較上月{rev_status} <b>{rev_mom:.2f}%</b>。</li>
                <li><b>技術形態：</b>股價目前處於 <b>{trend}</b>。</li>
                <li><b>綜合評價：</b>基本面{'正向' if rev_mom > 0 else '保守'}且技術面{'偏多' if trend == "多頭排列" else '中性'}。</li>
            </ul>
        </div>
        """
        return insight
    except:
        return "<div class='ai-insight'>⚠️ 無法產生 AI 總結，數據計算中...</div>"
...
try:
    ...
    with tab1:
        if not df_p.empty:
            ...
            st.metric(f"{ticker_input} 最新價格", f"{curr_p:.2f}", f"{curr_p-prev_p:.2f}")
    ...
    with tab3:
        st.subheader(f"🔥 即時情報 (台北時間: {now_tw.strftime('%m/%d')})")
        if news_list:
            for n in news_list[:12]:
                ...
                st.markdown(f"""
                <div class="news-card">
                    <div style="font-weight: bold;"><a href="{n.get('link', '#')}" target="_blank" style="text-decoration: none; color: #1f77b4;">{n.get('title', '無標題')}</a></div>
                    <div style="font-size: 12px; color: #666; margin-top: 5px;">來源: {n.get('publisher', '未知')} | 發佈: {pub_date}</div>
                </div>
                """, unsafe_allow_html=True)
        else:
