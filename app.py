...
def get_pro_data(ticker):
    ...
    # 2. 即時新聞 (增加抓取量，確保覆蓋到今天)
    df_news = dl.taiwan_stock_news(stock_id=stock_id, start_date=(now_tw - timedelta(days=14)).strftime('%Y-%m-%d'))
...
try:
    ...
    with tab2:
        ...
        if not df_r.empty:
            ...
            display_df.columns = ['年度', '月份', '營收', '月增率 (%)']
            st.dataframe(display_df.style.format({'營收': '{:,.0f}', '月增率 (%)': '{:.2f}%'}), use_container_width=True)
        else:
            st.info("暫無財報數據。")

    with tab3:
        st.subheader(f"🔥 即時情報 (更新於 {now_tw.strftime('%H:%M')})")
        if not df_n.empty:
            ...
            st.warning("⚠️ 過去 14 天查無重大訊息。如果是熱門股，請檢查網路連線或稍後再試。")
